...
import type { ApiResponse, FredCategorySeriesResponse, FredSearchOrderBy, FredSortOrder, FredFilterVariable } from '@/lib/fred/types';
  ...
  try {
    ...
    if (!category_id) {
      return NextResponse.json<ApiResponse<null>>(
        { data: null, error: 'category_id is required', cached: false, timestamp: new Date().toISOString() },
        { status: 400 }
      );
    }

    const client = getFredClient();
    const { data, cached } = await client.getCategorySeries({
    ...
    return NextResponse.json<ApiResponse<FredCategorySeriesResponse>>({
      data,
      error: null,
      cached,
