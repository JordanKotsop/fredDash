...
export async function exportChartToPdf(
  ...
  // generated by Tailwind v4.  We sanitise every <style> block in the
  // cloned document so html2canvas only sees hex/rgb colours.
  const canvas = await html2canvas(chartElement, {
    ...
    onclone: (clonedDoc) => {
      // Replace lab()/oklch()/oklab()/lch() in every <style> block
      // with a safe hex fallback so html2canvas can parse the CSS.
      const MODERN_COLOR_RE = /(?:oklch|oklab|lab|lch)\([^)]+\)/gi;
      for (const style of clonedDoc.querySelectorAll('style')) {
        if (style.textContent && MODERN_COLOR_RE.test(style.textContent)) {
          ...
          style.textContent = style.textContent.replace(MODERN_COLOR_RE, '#888');
        }
      }

      // Force light mode vars + colors
